...
public class Palya_Rajzolo extends JPanel {
	...
	private static Sin_Graf[][] fields;
	 ...
	 * 
	 * @param s
	 *            A modelbeli Sin elem
	 * @param x
	 *            vizszintes koordináta (1-20) (ha xy=true)
	 * @param y
	 *            függöleges koordináta (1-14) (ha xy=true)
	 * @param allapot
	 *            az elem állapota (lsd. konstruktor)
	 */
	 ...
	 * 
	 * @param s
	 *            A modelbeli SpecialisHely elem
	 * @param x
	 *            vizszintes koordináta (1-20) (ha xy=true)
	 * @param y
	 *            függöleges koordináta (1-14) (ha xy=true)
	 ...
	 * 
	 * @param s
	 *            A modelbeli Keresztezodes elem
	 * @param x
	 *            vizszintes koordináta (1-20) (ha xy=true)
	 * @param y
	 *            függöleges koordináta (1-14) (ha xy=true)
	...
	/**
	 * berakja a tarolójaból az összes panelt a megfelelő sorrendben Az üres
	 * helyekre üres (háttérszínű) panelt rak be
	 * 
	 */
	public void ready() {
		for (int i = 0; i < 14; i++) {
	...
	public static int GetDistanceBetweenTwoSpecHely(SpecialisHely s1, SpecialisHely s2){
		int sOneX = 0;
		int sOneY = 0;
		int sTwoX = 0;
		int sTwoY = 0;
		for (int i = 0; i < 14; i++) {
			for (int t = 0; t < 20; t++) {
				...
				if (fields[i][t] != null && fields[i][t].getSinem() != null) {
					if ((fields[i][t]).getSinem().equals(s1)) {
						sOneX = i;
						sOneY = t;
					} else if ((fields[i][t]).getSinem().equals(s2)) {
						sTwoX = i;
						sTwoY = t;
					}
				}
		...
		double distance = Math.sqrt((sTwoX - sOneX) * (sTwoX - sOneX) + (sTwoY - sOneY) * (sTwoY - sOneY));
		return (int)distance;
	}
}
