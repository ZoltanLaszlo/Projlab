...
public class SpecialisHely extends Sin{
	...
	public Boolean akcio() throws TooManyKapuException{
		if(kapum!=null){
			if(kapum.getMasikKapu()!=null){
				if (kapum.ures() && kapum.getMasikKapu().ures()){
					kapum.getMasikKapu().ad((Kapu)null);
					felkapu=kapum.getMasikKapu();
					kapum=null;
					kapukszama--;
					return true;
				}
			...
			if(kapukszama==0){
				...
				felkapu.ad(kapum);
				int distance = Palya_Rajzolo.GetDistanceBetweenTwoSpecHely(this, kapum.getMasikKapu().getSpecHelyem());
				kapum.Kapubeallitas(distance);
				felkapu.Kapubeallitas(distance);
				kapukszama=2;
				return true;
			}
			else {
	...
	**/
	...
	public boolean vanSajatKapum() { 	//hozzáadva ba 05.06
		if (kapum != null) {
			return true;
		}
		return false;
	...
	public Sin kovetkezo(Sin elozo) throws EndGameException{
		...
		if (elozo.equals(this.elozo)){
			if(vankapu()){
				...
				kapum.belep(kocsi);
				//leptem=true;
				return kapum.getMasikKapu().getSpecHelyem();
			}
			else{
				return kovetkezo;
			}
