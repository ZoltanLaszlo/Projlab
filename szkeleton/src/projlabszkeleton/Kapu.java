...
public class Kapu implements Elem{
	 ...
	 * akkor létre kell hozni a hosszal megegyező méretű tárolót
	 * 
	...
	/**
	 *Léptetjük az alagútban lévő vonatokat, 
		...
		if(!ures()){
			if(belepokocsik[0]!=null){ //ha az alagút elején egy kocsi áll
				if(belepokocsik[0].mogotte!=null){  //ha ez a kocsi nem az utolsó, hanem van még utána kocsi
					...
					if(belepokocsik[0].mogotte.mogotte != null){	//ha van még a kocsi mögött több kocsi akkor azt ráléptetjük az előbb felszabadított speciális helyre
						belepokocsik[0].mogotte.mogotte.kocsilepj(belepokocsik[0].mogotte.sinem());
					}
				}
				belep(belepokocsik[0].mogotte); //léptetjuk az alagútban lévő kocsikat, úgy hogy megadjuk az elejére kerülő kocsit
			}
			else{
				belep(null); //léptetjuk az alagútban lévő kocsikat, úgy, hogy az elejére nem kerül Kocsi
			}
		}
	}
	
	/**
	 *a kötelező implementálás végett, nem csinál semmit
	 *
	 */	
	@Override
	 ...
	 *
	 *@param helyem Az őt tartalmazó speciális hely
	 */	
